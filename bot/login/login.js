...
async function getAppStateToLogin(loginWithEmail) {
        ...
        if (!accountText || accountText.trim() == "" || accountText.trim() == "[]") {
                log.warn("LOGIN FACEBOOK", "No account found in Shourov.dev.txt. Skipping login, but starting dashboard and loading commands...");
                return null;
        }
        try {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        if (error) {
                                ...
                                // —————————————————— LOAD DATA ——————————————————— //
                                const { threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, sequelize } = await require(process.env.NODE_ENV === 'development' ? "./loadData.dev.js" : "./loadData.js")(null, createLine);
                                // —————————————————— LOAD SCRIPTS —————————————————— //
                                await require(process.env.NODE_ENV === 'development' ? "./loadScripts.dev.js" : "./loadScripts.js")(null, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, createLine);
                                log.success("SYSTEM", "Dashboard and commands loaded successfully without Facebook login.");
